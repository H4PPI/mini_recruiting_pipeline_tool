...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const candidate = await prisma.candidate.findUnique({ where: { id }, select: { sourceBlobPath: true } });
    if (!candidate || !candidate.sourceBlobPath) {
    ...
    const result = await get(candidate.sourceBlobPath, { access: "private" });
